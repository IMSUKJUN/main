...
CPV.config = {
  ...
  // 맨 위에 닿은 뒤, 이전 내역이 다 나올 때까지 기다리는 값들.
  // 맨 위에 닿아도 바로 안 나온다. 앱이 그때부터 받아오기 시작한다.
  topQuiet: 6,           // 이만큼 연속으로 안 변해야 진짜 맨 위로 본다(약 1초)
  topMax: 30000,         // 그래도 안 끝나면 이 시간(ms) 뒤에는 그냥 시작한다
  topStuck: 12,          // 이만큼 밀어도 위로 안 가면 붙들지 않고 넘어간다
